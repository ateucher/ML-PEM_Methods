...
predict_landscape <- function(model, cov, tilesize = 500,
 ...
 #
  # generate map
  model = paste0("Deception_AOI/2_sample_design/stage1_Analysis/models/",i,"/model_quick.rds")
  cov = rast_list_s
  tilesize = 500
   outDir = paste0("Deception_AOI/2_sample_design/stage1_Analysis/models/",i,"/quick_map")
  ...
  if (length(setdiff(mod$features, n)) != 0) { ## tests if all model features are found in the cov list
    ...
    for (i in 1:nrow(tiles)) {    ## testing first 2 tiles       ##nrow(tiles)) {
      ...
      ## * Test if tile is empty  -------
        ...
        # naTest <- naTest[,1]
        # sapply(rsf.df, function(x) all(is.na(x)))
        #
