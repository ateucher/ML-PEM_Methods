...
predict_map <- function(model, cov, tilesize = 500,
                        outDir = "./predicted") {
  
  # testing
  #
  # generate map
      # model = paste0(out_dir,"/",mname,"/model.rds")
      # cov = rast_list
      # tilesize = 500
  ...
  if (length(setdiff(mod$features, n)) != 0) { ## tests if all model features are found in the cov list
    ...
    for (i in 1:nrow(tiles)) {    ## testing first 2 tiles       ##nrow(tiles)) {
     # i = 8
      ...
      t_bb <- st_as_sf(t, crs = 3005)
      
      #r_template <- raster::crop(raster(cov[1]),extent(t))
      #r <- mask(r, as(bgc_filter,'Spatial'))
      
      
      # create a raster template in stars to convert back to stars object 
      #rtemplate <- stars::st_as_stars(r_template)
      ...
      if(any(sapply(rsf.df, function(x) all(is.na(x))) == TRUE)){
        ...
        pred <- predict(mod, newdata = rsf.df, type = "response")
        ...
        r_out <- r_out %>% dplyr::select(response)
        ...
        for (j in 1:length(keep)) {
          ...
          out <- stars::st_rasterize(r_out[j],
                                     template = st_as_stars(st_bbox(t_bb))) #rtemplate)
    ...
    for (k in keep) {
      # get list of tiles
      k = "response" # testing
      #outDir <- file.path(outDir, "predicted", "response")
      r_tiles <- list.files(#paste(outDir, k, sep = "/"),
                            file.path(outDir,"response"),
                            pattern = ".tif$",
                            full.names = TRUE)
